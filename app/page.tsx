...
import { BarChart3, Beer, Boxes, ChevronRight, CircleDollarSign, ClipboardList, Clock3, Edit, Eye, EyeOff, LogOut, Menu, PackagePlus, Plus, Search, Settings, ShoppingCart, Sparkles, Users, X } from 'lucide-react';
        ...
        <div className="p-8 sm:p-12">
          ...
          <div className="mb-8">
            ...
            <h2 className="text-3xl font-semibold tracking-tight">
              {mode === 'login' ? 'Sign in to your bar' : 'Create your account'}
            </h2>
            <p className="mt-2 text-sm text-slate-400">
              {mode === 'login' ? 'Pick up where the shift left off.' : 'New accounts start as staff.'}
            </p>
          </div>
...
function AddProduct({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  function handleSellingPriceChange(val: string) {
    setSellingPrice(val);
    if (val && !costPrice) {
      setCostPrice(String(Math.round(Number(val) * 0.6)));
    }
  }
  ...
  async function save(e: React.FormEvent) {
    ...
    try {
      await api.products.create({
        ...
        costPrice: Number(costPrice) || Math.round(Number(sellingPrice) * 0.6),
      ...
      <form onSubmit={save} className="w-full max-w-md rounded-2xl border border-white/10 bg-[#10211c] p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <label className="block text-sm text-slate-300">
              Cost price (NGN ₦)
              <input value={costPrice} onChange={e => setCostPrice(e.target.value)} type="number" className="field mt-2" placeholder="900" />
            </label>
            <label className="block text-sm text-slate-300">
              Selling price (NGN ₦)
              <input value={sellingPrice} onChange={e => handleSellingPriceChange(e.target.value)} required type="number" className="field mt-2" placeholder="1500" />
            </label>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <label className="block text-sm text-slate-300">
            ...
            <label className="block text-sm text-slate-300">
              Reorder threshold
              <input value={reorderThreshold} onChange={e => setReorderThreshold(e.target.value)} required type="number" className="field mt-2" placeholder="5" />
            </label>
          </div>
          {error && <p className="text-sm text-red-300">{error}</p>}
        </div>
        <div className="mt-7 flex justify-end gap-3">
          <button type="button" onClick={onClose} className="button-quiet">Cancel</button>
          <button disabled={busy} className="button-primary">{busy ? 'Saving…' : 'Save product'}</button>
        </div>
      </form>
    </div>
  );
      ...
      <form onSubmit={save} className="w-full max-w-md rounded-2xl border border-white/10 bg-[#10211c] p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="mb-6 flex items-start justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">Modify details for {product.name}.</p>
          </div>
          <button type="button" onClick={onClose} className="text-slate-500 hover:text-white"><X size={20} /></button>
        </div>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <label className="block text-sm text-slate-300">
              Cost price (NGN ₦)
              <input value={costPrice} onChange={e => setCostPrice(e.target.value)} required type="number" className="field mt-2" placeholder="900" />
            </label>
            <label className="block text-sm text-slate-300">
              Selling price (NGN ₦)
              <input value={sellingPrice} onChange={e => setSellingPrice(e.target.value)} required type="number" className="field mt-2" placeholder="1500" />
            </label>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <label className="block text-sm text-slate-300">
              Stock quantity
              <input value={stockQuantity} onChange={e => setStockQuantity(e.target.value)} required type="number" className="field mt-2" placeholder="24" />
            </label>
            <label className="block text-sm text-slate-300">
              Reorder threshold
              <input value={reorderThreshold} onChange={e => setReorderThreshold(e.target.value)} required type="number" className="field mt-2" placeholder="5" />
        ...
        <div className="mt-7 flex justify-end gap-3">
          <button type="button" onClick={onClose} className="button-quiet">Cancel</button>
          <button disabled={busy} className="button-primary">{busy ? 'Saving…' : 'Save changes'}</button>
        </div>
      </form>
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        ...
        <button onClick={() => setShowAdd(true)} className="button-primary"><PackagePlus size={17} /> Add product</button>
      </div>
          ...
          <table className="w-full text-left text-sm">
            <thead className="text-xs uppercase tracking-wider text-slate-500">
              <tr>
                ...
                <th className="px-5 py-4 font-medium">Selling Price</th>
                <th className="px-5 py-4 font-medium">In stock</th>
              ...
              {filtered.map(p => {
                ...
                return (
                  <tr key={p.id} className="border-t border-white/5 transition hover:bg-white/[0.025]">
                    ...
                    <td className="px-5 py-4 font-medium text-slate-300">{money.format(p.costPrice)}</td>
                    <td className="px-5 py-4 font-medium text-slate-200">{money.format(p.sellingPrice)}</td>
                    <td className="px-5 py-4 text-slate-300">{p.stockQuantity}</td>
                    <td className="px-5 py-4">
                    ...
                    <td className="px-5 py-4 text-right">
                      <button onClick={() => setEditingProduct(p)} className="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-xs font-semibold bg-emerald-400/10 text-emerald-300 hover:bg-emerald-400/20 transition">
                        <Edit size={13} />
                        Edit
                      </button>
                    </td>
...
function Pos({ products, refresh }: { products: Product[]; refresh: () => void }) {
  ...
  const [posMobileView, setPosMobileView] = useState<'menu' | 'cart'>('menu');
  ...
  return (
    <div className="space-y-6">
      ...
      {/* POS view selector on mobile/tablet */}
      <div className="flex xl:hidden rounded-xl bg-white/[0.03] p-1 border border-white/5 w-full">
        <button
          onClick={() => setPosMobileView('menu')}
          className={`flex-1 rounded-lg py-2.5 text-xs font-semibold transition text-center ${posMobileView === 'menu' ? 'bg-emerald-400 text-slate-950' : 'text-slate-400 hover:text-white'}`}
        >
          Menu
        </button>
        <button
          onClick={() => setPosMobileView('cart')}
          className={`flex-1 rounded-lg py-2.5 text-xs font-semibold transition text-center relative ${posMobileView === 'cart' ? 'bg-emerald-400 text-slate-950' : 'text-slate-400 hover:text-white'}`}
        >
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_360px]">
        {/* Menu Column */}
        <div className={`${posMobileView === 'menu' ? 'block' : 'hidden'} xl:block`}>
        ...
        {/* Cart Column */}
        <div className={`${posMobileView === 'cart' ? 'block' : 'hidden'} xl:block panel h-fit p-5 xl:sticky xl:top-6`}>
          ...
          <div className="space-y-3 border-t border-white/5 pt-5 text-sm">
            ...
            <button onClick={checkout} disabled={!cart.length} className="button-primary mt-3 w-full">
